...
/**
 * Overrides the arrayadapter to display a list of suggestions retrieved from a
 * geocoder.
 * <p>
...
public class FindPlaceAdapter extends ArrayAdapter<String> {
    ...
    public FindPlaceAdapter(final Context context, final int resource, final int txtViewResId) {
    ...
    /**
     * Replaces ArrayAdapter's filter with one that retrieves suggestions from
    ...
    private class GeoFilter extends Filter {
        ...
        /**
         * Perform filtering by using the character sequence as a search string
         * for a geocoder and the existing sql store of previous addresses.
         */
            ...
            if (ch == null) {
                ...
                try {
                    ...
                    for (Address address : addresses) {
                        results.add(LocationUtils.asString(address));
                    }

                    res.count = results.size();
                    res.values = results;
                } catch (IOException e) {
                    res.count = -1; // pass result back to ui thread to show
                    // message
                }
